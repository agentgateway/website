...
| [Idle timeout]({{< link-hextra path="/resiliency/timeouts/idle/" >}})  | An idle timeout is the time when the proxy terminates the connection to a downstream or upstream service if there are no active streams.| <ul><li>{{< reuse "agw-docs/snippets/policy.md" >}} </li></ul> | <ul><li>Gateway listener</li></ul> | 
