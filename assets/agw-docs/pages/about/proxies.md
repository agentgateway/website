...
| --- | --- | --- |
| {{< icon "kgateway" >}} kgateway | API, ingress, egress, service mesh | The kgateway project includes its own proxy that is based on Envoy, an L3/L4/L7 network proxy. Beyond Envoy, kgateway provides a set of extensions for advanced configuration, security, and traffic management features. You can also integrate kgateway with the Istio service mesh in sidecar and ambient modes. For more information, see the [kgateway FAQs]({{< link-hextra path="/documentation/faqs/">}}) and [Envoy docs](https://www.envoyproxy.io/docs/envoy/latest/intro/what_is_envoy). |
| {{< icon "agentgateway" >}} agentgateway | HTTP, gRPC, API gateway, AI, A2A, MCP, LLM, Inference Extension | Agentgateway is an enterprise-grade gateway data plane for both standard L7 traffic and AI-first workloads: the same proxy fronts HTTP/gRPC APIs and provides first-class connectivity for agents, tools, and LLMs in any environment. <ul><li>For Kubernetes deployments, refer to [agentgateway.dev/docs/kubernetes/](https://agentgateway.dev/docs/kubernetes/)</li><li>For non-Kubernetes environments such as standalone or on-prem deployments, refer to [agentgateway.dev/docs/standalone/](https://agentgateway.dev/docs/standalone/)</li></ul>|
