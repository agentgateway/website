...
For more information about how kgateway integrates with agentgateway, see the [Architecture]({{< link-hextra path="/documentation/about/architecture/">}}) topic.

For more information about agentgateway resources, see the [Agentgateway upstream docs]({{< link-hextra path="/documentation/about/">}}).
...
- Attaching an {{< reuse "agw-docs/snippets/policy.md" >}} to a particular route via the `ExtensionRef` filter is not supported. Instead, use the [HTTPRoute rule attachment option]({{< link-hextra path="/documentation/about/policies/trafficpolicy/#attach-to-rule" >}}) to apply a policy to an individual route, which uses named HTTPRoute rules. Named rules are available in the standard channel of Kubernetes Gateway API 1.4 or later; on Gateway API 1.3, they require the experimental channel.
...
- External processing (extProc) as part of the {{< reuse "agw-docs/snippets/policy.md" >}} is not supported.
- Configuring your agentgateway proxy as a Dynamic Forward Proxy (DFP) is currently not supported.
- [Header modifier filters]({{< link-hextra path="/documentation/traffic-management/header-control/">}}) in {{< reuse "agw-docs/snippets/policy.md" >}} are not supported for agentgateway proxies. You can still use header modifier filters in the Gateway API-native HTTPRoutes.
