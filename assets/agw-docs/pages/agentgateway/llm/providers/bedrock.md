...
> [!NOTE]
> This page configures Bedrock as an LLM provider for direct model invocation. To route to a deployed Amazon Bedrock AgentCore agent runtime instead, which is a separate service with its own invocation endpoint, see [Connect to AWS Bedrock AgentCore]({{< link-hextra path="/agent/agentcore/" >}}).
>
