...
3. To use AWS IAM SSO: Configure your profile with [`aws configure sso`](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-sso.html), run [`aws sso login`](https://docs.aws.amazon.com/cli/latest/userguide/sso-using-commands.html), and make sure the workload can use that profile (for example with `AWS_PROFILE`). Later when you create the {{< reuse "agw-docs/snippets/backend.md" >}}, omit `policies.auth` so the proxy uses implicit AWS credentials, the default SDK credential chain which includes SSO.{{< /version >}}
...
2. Create an {{< reuse "agw-docs/snippets/backend.md" >}} resource to configure your LLM provider. Make sure to reference the secret that holds your credentials to access the LLM. 
   ...
   | `policies.auth` | Provide the credentials to use to access the Amazon Bedrock API. The example refers to the secret that you previously created. To use implicit credentials from the workload or environment instead (for example IRSA), omit the `auth` settings. {{< version exclude-if="1.1.x" >}}AWS IAM Identity Center (SSO) profiles are supported on the same implicit credential chain.{{< /version >}}|
