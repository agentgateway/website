...
The {{< reuse "agw-docs/snippets/gatewayparameters.md" >}} API provides two ways to customize your proxy:
...
Overlays are useful when you need to customize resources in ways that are not directly exposed through the {{< reuse "agw-docs/snippets/gatewayparameters.md" >}} configs. Common use cases include:
...
1. Reads configs from your {{< reuse "agw-docs/snippets/gatewayparameters.md" >}} (like `image`, `logging`, `resources`)
...
{{< callout type="info" >}}
**Why null requires server-side apply:** Without server-side apply, Kubernetes strips null values from the CRD before storing it. Your {{< reuse "agw-docs/snippets/gatewayparameters.md" >}} resource appears to save successfully, but the null values are silently dropped. Always use `kubectl apply --server-side` when your overlays contain null values.
{{< /callout >}}
...
You can attach {{< reuse "agw-docs/snippets/gatewayparameters.md" >}} to either a GatewayClass (shared by all Gateways using that class) or to an individual Gateway. When both are specified, they are processed in the following order:
...
The following recipes demonstrate common overlay patterns. Each recipe shows a complete {{< reuse "agw-docs/snippets/gatewayparameters.md" >}} resource that you can adapt to your needs.
...
kubectl get deployment my-gateway -n {{< reuse "agw-docs/snippets/namespace.md" >}} -o yaml

# View the generated Service
kubectl get service my-gateway -n {{< reuse "agw-docs/snippets/namespace.md" >}} -o yaml

# View the generated ServiceAccount
kubectl get serviceaccount my-gateway -n {{< reuse "agw-docs/snippets/namespace.md" >}} -o yaml
```
...
1. Check pod events:
   ```bash
   kubectl describe pod -l app.kubernetes.io/name=my-gateway -n {{< reuse "agw-docs/snippets/namespace.md" >}}
   ```

2. Check container logs:
   ```bash
   kubectl logs deployment/my-gateway -n {{< reuse "agw-docs/snippets/namespace.md" >}}
   ```
...
Check that your {{< reuse "agw-docs/snippets/gatewayparameters.md" >}} resource contains your overlay as expected:

```bash
kubectl get {{< reuse "agw-docs/snippets/gatewayparameters.md" >}} my-params -n {{< reuse "agw-docs/snippets/namespace.md" >}} -o yaml
```
