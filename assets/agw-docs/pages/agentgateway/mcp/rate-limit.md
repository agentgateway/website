...
{{% version exclude-if="1.0.x,1.1.x,1.2.x,1.3.x,1.4.x,1.5.x" %}}| Give each client its own limit | Add `local[].key`, such as `jwt.sub`. See [Per-client limits](#per-client). |{{% /version %}}
