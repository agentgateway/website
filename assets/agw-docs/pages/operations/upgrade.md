...
- You can skip patch versions within the same minor release. For example, you can upgrade from version {{< reuse "agw-docs/versions/short.md" >}}.0 to {{< reuse "agw-docs/versions/n-patch.md" >}} directly, and skip the patch versions in between.
...
- Always upgrade to the latest patch version of the target minor release. Do not upgrade to a lower patch version, such as {{< reuse "agw-docs/versions/short.md" >}}.0, {{< reuse "agw-docs/versions/short.md" >}}.1, and so on.
...
2. Check that your underlying infrastructure platform, such as Kubernetes, and other dependencies run supported versions for the kgateway version that you want to upgrade to.
   1. Review the [supported versions]({{< link-hextra path="/reference/versions/">}}) for dependencies such as Kubernetes, Istio, Helm, and more.
...
3. Set the version to upgrade kgateway to in an environment variable, such as the latest patch version (`{{< reuse "agw-docs/versions/n-patch.md" >}}`) .
   ```sh
...
1. Upgrade `{{< reuse "agw-docs/snippets/cli-name.md" >}}` to the new version. Note that this command only updates the CLI binary version, and does not upgrade your kgateway installation.
   ```shell
   {{< reuse "agw-docs/snippets/cli-name.md" >}} upgrade --release v${NEW_VERSION}
   ```

2. Verify that the **client** version matches the version you installed.
   ```shell
   {{< reuse "agw-docs/snippets/cli-name.md" >}} version
   ```
...
1. Decide on the {{< reuse "agw-docs/snippets/k8s-gateway-api-name.md" >}} version that you want to use. 
   ...
   * Check if you need to install the [experimental channel for the features that you want to use]({{< link-hextra path="/reference/versions/#experimental-features">}}).
