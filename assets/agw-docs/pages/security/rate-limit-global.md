...
|-------|----------|-------------|
| `backendRef` | Yes | Reference to the rate limit service. Supports `Service` or `Backend` kind.{{< version exclude-if="1.4.x,1.3.x,1.2.x,1.1.x,1.0.x" >}} Required unless you set `url` instead. Set exactly one of the two.{{< /version >}} |
| `backendRef.name` | Yes | Name of the Service or Backend. |
| `backendRef.namespace` | Yes | Namespace where the service lives. |
| `backendRef.port` | Yes | gRPC port (typically 8081). |{{< version exclude-if="1.4.x,1.3.x,1.2.x,1.1.x,1.0.x" >}}
