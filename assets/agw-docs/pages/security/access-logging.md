...
To filter which requests are logged or customize log fields, see [Filter access logs](#filter-access-logs) and [Add and remove log attributes](#add-and-remove-log-attributes). To export access logs to an external backend over OTLP, see [Export logs over OTLP]({{< link-hextra path="/documentation/observability/access-logs/export/" >}}).
