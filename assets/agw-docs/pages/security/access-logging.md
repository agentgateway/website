...
5. Check the access log file in the gateway pod and verify that you see a string entry for each request that you sent to the httpbin app. 
   ...
   ```sh
   kubectl -n {{< reuse "agw-docs/snippets/namespace.md" >}} exec -it deploy/http -- cat /dev/default-access-logs.txt
   ```
...
1. Create or get the details of the gRPC service. The following example creates a simple `log-test` service in the {{< reuse "agw-docs/snippets/namespace.md" >}} namespace that listens on port 50051.
...
5. Get the logs of the gRPC service and verify that you see an entry for the matching requests that you sent to the httpbin app. 
   ...
   ```sh
   kubectl -n {{< reuse "agw-docs/snippets/namespace.md" >}} logs deployments/log-test | tail -1 | jq --sort-keys
   ```
