Exchange the caller's credential for a backend-scoped token with the [RFC 7523](https://datatracker.ietf.org/doc/html/rfc7523) JWT bearer grant, configured on an {{< reuse "agw-docs/snippets/policy.md" >}}.
...
2. In another terminal, mint the inbound credential. Mint a token from the `idp` realm as `idp-app`; the gateway presents this as the RFC 7523 `assertion` to the `backend-oauth` realm, which trusts the `idp` realm. Tokens expire, so re-mint if you come back later.
...
5. Decode the token's payload to confirm the exchange.
   ...
   The decoded token was issued for the target audience (`aud`), and its authorized party (`azp`) is the gateway's client (`requester-client`), not the client that minted the inbound credential. The standard token exchange grant produces the same exchanged token from a different inbound credential.
...
To verify this variant, mint a user access token from your Entra tenant as the inbound credential, send it through the gateway, and inspect the exchanged on-behalf-of token that the gateway forwards, as in [Verify the exchange](#verify-the-exchange).
