...
1. Create an {{< reuse "agw-docs/snippets/policy.md" >}} with JWT authentication configuration.
   ...
   | `jwks.remote.url` | The full address of the JWKS endpoint, including its path. Use this field instead of `backendRef` and `jwksPath` to point at the identity provider directly, without creating an intermediate Kubernetes object. When the scheme is `https`, backend TLS is enabled automatically, so you do not need a separate BackendTLSPolicy. Mutually exclusive with `backendRef`, and you cannot set `jwksPath` alongside it.<br><br>Example value: `https://my-org.okta.com/oauth2/default/v1/keys` |{{< /version >}}
