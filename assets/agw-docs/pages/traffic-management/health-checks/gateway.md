...
3. Send a request to the health check path. Because Envoy is in a draining state, the `503 Service Unavailable` message is returned.
   ...
   x-envoy-upstream-healthchecked-cluster: http.{{< reuse "/agw-docs/snippets/namespace.md" >}}
...
```sh
kubectl delete HTTPListenerPolicy healthcheck -n {{< reuse "/agw-docs/snippets/namespace.md" >}}
```
