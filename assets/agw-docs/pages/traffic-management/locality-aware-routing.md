Reduce cross-zone traffic costs and latency by routing requests to nearby endpoints, with automatic failover to other localities when local endpoints are unavailable.

## About locality-aware routing {#about}
...
{{< callout type="info" >}}
Kubernetes 1.33 and later split `PreferClose` into more granular values, `PreferSameZone` and `PreferSameNode`. Older API servers might reject these values. Use `PreferClose` for the broadest compatibility.
{{< /callout >}}
...
In this task, you deploy three backend instances that represent three localities, and then enable `PreferClose` on the Service so that the gateway prefers same-zone endpoints and falls back to other zones or regions only when needed.
...
4. Capture each backend pod's IP and create a WorkloadEntry that overrides its locality. The labels on each WorkloadEntry match the Service selector, so {{< reuse "agw-docs/snippets/agentgateway.md" >}} treats them as endpoints of `locality-svc`.
...
1. Restore the same-zone and same-region WorkloadEntries that you deleted in the previous task. Run the same commands as in step 4 of the failover task to capture pod IPs and reapply all three WorkloadEntries.

{{< doc-test paths="locality-aware-routing" >}}
# Recreate the WorkloadEntries that the failover task deleted, so the strict-mode
# example has endpoints to filter against.
ZONE_A_IP=$(kubectl get pod -n agentgateway-locality -l app=backend-zone-a -o jsonpath='{.items[0].status.podIP}')
ZONE_B_IP=$(kubectl get pod -n agentgateway-locality -l app=backend-zone-b -o jsonpath='{.items[0].status.podIP}')
REGION_B_IP=$(kubectl get pod -n agentgateway-locality -l app=backend-region-b -o jsonpath='{.items[0].status.podIP}')
