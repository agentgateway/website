...
1. [Install {{< reuse "/agw-docs/snippets/kgateway.md" >}}]({{< link-hextra path="/documentation/quickstart/">}}) in a cluster.
...
Create an HTTPS listener so that the gateway can route gRPC traffic. GRPCRoute requires HTTPS listeners for TLS termination. For more information, see the [HTTPS listener guide]({{< link-hextra path="/documentation/setup/listeners/https/">}}).
...
2. Create a Gateway resource with an HTTPS listener.
   ...
   | Setting | Description |
   |---------|-------------|
   | `spec.gatewayClassName` | The name of the Kubernetes GatewayClass. When you set up {{< reuse "/agw-docs/snippets/kgateway.md" >}}, a default GatewayClass is set up for you. |
...
3. Check the status of the Gateway.
   ...
   ```bash
   kubectl get gateway grpc-gateway -n {{< reuse "agw-docs/snippets/namespace.md" >}} -o yaml
   ```
...
2. Verify that the GRPCRoute is applied successfully.
   ...
   ```bash
   kubectl get grpcroute grpc-echo-route -n {{< reuse "agw-docs/snippets/namespace.md" >}} -o yaml
   ```
