Associate an IAM role with a gateway proxy service account, and configure {{< reuse "/agw-docs/snippets/kgateway.md" >}} to use that service account to access AWS Lambda.

## About

Amazon Web Services (AWS) offers the ability to associate an IAM role with a Kubernetes service account, also known as creating an IRSA. {{< reuse "/agw-docs/snippets/agentgateway-capital.md" >}} supports discovering and invoking AWS Lambda functions by using an IRSA. For more information, see the [AWS documentation](https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html).
...
**{{< reuse "/agw-docs/snippets/agentgateway-capital.md" >}} resources**:
* Install {{< reuse "/agw-docs/snippets/kgateway.md" >}}
...
4. Use an IAM role to associate the policy with the Kubernetes service account for the HTTP gateway proxy, which assumes this role to invoke Lambda functions. For more information about these steps, see the [AWS documentation](https://docs.aws.amazon.com/eks/latest/userguide/associate-service-account-role.html).
   1. Create the following IAM role. Note that the service account name `http` in the `{{< reuse "/agw-docs/snippets/namespace.md" >}}` namespace is specified, because in later steps you create an HTTP gateway named `http`.
      ```sh
      cat >role.json <<EOF
      {
        "Version": "2012-10-17",
        "Statement": [
          ...
          },
          {
            ...
            "Condition": {
              "StringEquals": {
                "${OIDC_PROVIDER}:sub": "system:serviceaccount:{{< reuse "/agw-docs/snippets/namespace.md" >}}:http"
              }
            }
          }
        ]
...
## Install {{< reuse "/agw-docs/snippets/kgateway.md" >}} {#install}
...
3. Check the status of the gateway to make sure that your configuration is accepted. Note that in the output, a `NoConflicts` status of `False` indicates that the gateway is accepted and does not conflict with other gateway configuration. 
   ```sh
   kubectl get gateway http -n {{< reuse "/agw-docs/snippets/namespace.md" >}} -o yaml
   ```
...
4. Verify that the `http` service account has the `eks.amazonaws.com/role-arn: arn:aws:iam::${AWS_ACCOUNT_ID}:role/lambda-role` annotation.
   ```sh
   kubectl describe serviceaccount http -n {{< reuse "/agw-docs/snippets/namespace.md" >}}
   ```

## Create a Lambda function

Create an AWS Lambda function to test {{< reuse "/agw-docs/snippets/kgateway.md" >}} routing.
...
At this point, {{< reuse "/agw-docs/snippets/kgateway.md" >}} is routing directly to the `echo` Lambda function using an IRSA!
...
1. Delete the `lambda` HTTPRoute and `lambda` Backend.
   ```sh
   kubectl delete HTTPRoute lambda -n {{< reuse "/agw-docs/snippets/namespace.md" >}}
   kubectl delete Backend lambda -n {{< reuse "/agw-docs/snippets/namespace.md" >}}
   ```
