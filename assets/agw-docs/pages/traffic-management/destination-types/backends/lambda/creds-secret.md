...
3. Confirm that {{< reuse "/agw-docs/snippets/kgateway.md" >}} correctly routes requests to Lambda by sending a curl request to the `echo` function.
...
At this point, {{< reuse "/agw-docs/snippets/kgateway.md" >}} is routing directly to the `echo` Lambda function!
...
2. Delete the `aws-creds` secret.
   ...
   ```sh
   kubectl delete secret aws-creds -n {{< reuse "/agw-docs/snippets/namespace.md" >}}
   ```
