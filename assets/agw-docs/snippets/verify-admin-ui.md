...
You can also confirm that the proxy received your configuration in the read-only [UI]({{< link-hextra path="/observability/ui/" >}}). In Kubernetes mode, the UI reflects the configuration that the agentgateway controller pushes to the proxy, so it is a quick way to check that your resources took effect.
