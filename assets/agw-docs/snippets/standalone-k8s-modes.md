{{< reuse "agw-docs/snippets/agentgateway-capital.md" >}} has two main deployment modes, called `standalone` and `kubernetes` throughout the documentation.
...
| User interface | <ul><li>UI that can edit config</li><li>`agentgateway` binary</li><li>`agctl` CLI</li></ul> | <ul><li>Read-only UI</li><li>Kubernetes CRs</li><li>`agctl` CLI</li></ul> |
...
**Standalone** mode refers to the agentgateway binary, a quick way to get started and run anywhere. You can deploy standalone agentgateway as a binary download, Docker container, or via a Helm chart for a simple Kubernetes Deployment. In standalone mode, the agentgateway UI can edit your configuration file, lending itself well to developers, simple ClickOps deployments, and more traditional, non-Kubernetes gateway infrastructure. For more information, make sure that you are in the **Docs > Standalone** section of the docs, `https://agentgateway.dev/docs/standalone/`.
