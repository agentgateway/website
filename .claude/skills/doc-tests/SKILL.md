...
|---|---|---|---|
| Tutorials with shell steps but no `test:` metadata | Just hasn't been wired up yet | Tag blocks with `paths=`, add hidden doc-test blocks for backgrounding/assertions | Content only |
...
| Integration guides with Docker-based deps (e.g. Langfuse, Prometheus) | Requires standing up extra services | Use hidden doc-test blocks to `docker compose up -d` and `trap 'docker compose down' EXIT`. Docker is already available in CI (kind clusters use it). Only worth the added test duration for high-traffic pages | Content only |
