...
func renderJSONSchemaWidget(doc crdDocument, widgetID string, titleOverride string) (string, bool) {
	m := doc.mapping()
...
func main() {
	...
	for _, doc := range docs {
		...
		if !isCRD(m) {
			...
			if widget, ok := renderJSONSchemaWidget(doc, widgetID, *titleOverride); ok {
