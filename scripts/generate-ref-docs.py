...
That is what archived docs need. Before this, no numbered directory was ever
written, so an archived version had nothing of its own to point at and its stub
reused .../helm/main/, which made every archived page display the values of the
in-development branch. Archiving a version now means pointing its stub at a
numbered directory that already exists, with no regeneration and no manual freeze
step -- the step that was missed for Helm and remembered for metrics, which is why
metrics-control-plane-22x.md exists but no matching helm/2.2.x snapshot does.

NOT BACKFILLED. Versions archived before this change (1.1.x through 1.4.x) have no
numbered directory, so their stubs still reuse .../helm/main/ and still show the
wrong values. Generating those requires running this script against each old tag.
...
def _freeze_dir(assets_path: str, link_version: str, doc_version: str, website_dir: str) -> None:
    """Copy the freshly generated link-version directory to its numbered equivalent.
    ...
    for entry in sorted(os.listdir(assets_path)):
        src = os.path.join(assets_path, entry)
