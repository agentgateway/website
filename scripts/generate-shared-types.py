...
class TypeInfo:
    ...
    validation: list = field(default_factory=list)
    source: str = ""  # Source directory name (e.g., 'shared', 'agentgateway', 'enterprisekgateway')
...
def find_referenced_types(agentgateway_file: Path, all_types: list[TypeInfo]) -> set[str]:
    """Find which shared types are referenced in the agentgateway docs."""
    if not agentgateway_file.exists():
        return set()
    
    content = agentgateway_file.read_text()
    ...
    def add_nested_refs(type_name: str, visited: set[str]):
        ...
        for field in type_info.fields:
            # Extract type names from the field type
...
def main():
    if len(sys.argv) < 3:
        print("Usage: generate_shared_types_docs.py <shared_dir> <doc_file> [source_dir...]")
    ...
    # Helper to detect if a directory is an enterprise source
    ...
    # we may need to document structs from OSS that aren't part of the enterprise CRD schema
