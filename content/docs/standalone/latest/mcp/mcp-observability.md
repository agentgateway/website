...
2. Configure your agentgateway proxy to emit traces and send them to the built-in OpenTelemetry collector agent. This example uses static tracing configuration. For per-route dynamic tracing configuration, see [Dynamic tracing]({{< link-hextra path="/reference/observability/traces#configure-dynamic-tracing" >}}). 
   ```yaml
   cat <<EOF > config.yaml
