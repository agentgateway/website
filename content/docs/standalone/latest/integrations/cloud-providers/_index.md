...
test: skip
aliases:
  - /docs/standalone/latest/integrations/platforms/
---

Agentgateway runs the same proxy and reads the same configuration file on every cloud provider, so none of the [installation methods]({{< link-hextra path="/setup/install/" >}}) change when you move to a cloud. What changes is the surrounding environment: which container runtime starts the process, how the process gets an identity, and where its configuration file comes from.
...
* Run standalone agentgateway as a Kubernetes Deployment with the [Helm chart]({{< link-hextra path="/setup/install/helm/" >}}). Your configuration file is the source of truth, rendered into a ConfigMap.
* Run the [Kubernetes control plane]({{< link-hextra path="/setup/install/kubernetes/" >}}) instead, which manages agentgateway proxies from Kubernetes custom resources and the Kubernetes Gateway API.
