...
test:
  xai:
  - file: ${versionRoot}/integrations/llm/providers/xai.md
    path: xai
---
...
#     (--validate-only), so `provider: xai` is a recognized provider and the
...
#     (https://api.x.ai/v1), so the table cannot drift from the product without
#     this test failing.
#
# WHAT THIS TEST DOES NOT VALIDATE (and why):
#   * "Example request" - external dependency; sending the curl request needs a
#     real xAI API key and bills a live completion, so the test uses a
