...
title: Axiom
weight: 5
description: Export standalone agentgateway traces, access logs, and metrics to Axiom.
test: skip
---
...
Add the following `frontendPolicies` section to your agentgateway configuration. The trace and access-log exporters use different datasets, so each exporter sets its own `x-axiom-dataset` header.
...
Agentgateway uses `/v1/traces` and `/v1/logs` by default for the respective OTLP/HTTP exporters. The configuration also adds the LLM input and output messages. Accessing `llm.prompt` and `llm.completion` causes agentgateway to inspect the request and response bodies, so omit these attributes if you do not want to export message content.
...
4. In Axiom, verify each signal.
   ...
   - Click **Stream**, select the traces Events dataset, and open the event with the trace ID. Click **Find trace** to open the trace waterfall after Axiom recognizes the dataset as an OpenTelemetry trace dataset. Axiom also automatically creates an **OpenTelemetry Traces** dashboard for the dataset.
...
- Confirm that `AXIOM_DOMAIN` contains only the ingest hostname, without `https://` or a trailing path.
- Set `randomSampling: true` while testing so that agentgateway starts a trace for every request.
- Confirm that the agentgateway metrics endpoint is available before starting the collector.
...
Stop the metrics collector by pressing `Ctrl+C`. If you started it in the background, remove the container.
