...
The following guard masks a Social Security Number wherever it appears, including in a tool result that an agent is about to feed back to the model.
...
{{< doc-test paths="regex" >}}
# WHAT THIS TEST VALIDATES:
#   * `scope` is accepted on a request guard with all four content categories.
#   * The camelCase enum values are the ones the standalone schema accepts.
# WHAT THIS TEST DOES NOT VALIDATE (and why):
#   * That a tool result is actually masked at request time -- the page shows no tool
#     call, and driving one needs a model that emits a tool call plus a tool server to
#     answer it, neither of which this guide sets up.
...
- **Only a request guard takes `scope`.** A response guard rejects the field. To scan the model's reply, use a response guard, which always inspects the message text.
- **Only `regex` and `bedrockGuardrails` guards support a non-default scope.** A `webhook`, `openAIModeration`, or `googleModelArmor` guard always inspects the system prompt and the messages. Setting `scope` on one of those fails validation with `only regex and bedrockGuardrails guards support a non-default scope`.
