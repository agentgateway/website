...
| Unit | US dollars or tokens | Tokens |
...
**A dollar cap is the main reason to choose a per-key budget.** A rate limit counts tokens, so it caps spend only as an estimate that drifts when prices or the model mix change. A per-key budget charges the realized cost of each request, so `USD` is a true spend cap.

Choose a rate limit instead to smooth burst traffic, because a token bucket refills continuously and a budget window does not, or when you run in Kubernetes mode.
...
Agentgateway holds the running count in memory and writes it to the database every five seconds, which keeps the database off the request path. The database also makes the count survive a restart, so every budget requires `config.database`.

## How budget windows work

A window is aligned to the Unix epoch, not to the first request that a key sends. A `1h` window follows UTC clock hours, a `24h` window starts at midnight UTC, and a `30d` window uses consecutive 30-day periods rather than calendar months. Every key with the same window length therefore resets at the same moment.
