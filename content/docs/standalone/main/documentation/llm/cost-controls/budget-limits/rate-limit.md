...
Unlike a [per-key budget]({{< link-hextra path="/documentation/llm/cost-controls/budget-limits/per-key/" >}}), a rate limit token budget needs no database, and it works in both standalone and Kubernetes mode. It counts tokens only, so it caps spend as an estimate rather than as a true dollar cap.

> [!NOTE]
> Agentgateway checks a token-based rate limit in two phases, at request time and at response time. The provider reports the completion token count only in the response. For details, see [Token-based rate limits]({{< link-hextra path="/documentation/configuration/resiliency/rate-limits/#token-based-rate-limits" >}}).
>
...
For the full local rate limit walkthrough, see [Virtual key management]({{< link-hextra path="/documentation/llm/cost-controls/virtual-keys/#configure-token-budgets" >}}).
...
A token budget approximates spend, but it drifts as prices or the model mix change. To cap spend in dollars directly, use a [per-key dollar budget]({{< link-hextra path="/documentation/llm/cost-controls/budget-limits/per-key/" >}}) instead.
...
- Cap spend in dollars with [per-key dollar or token budgets]({{< link-hextra path="/documentation/llm/cost-controls/budget-limits/per-key/" >}}).
- Attribute usage to teams and keys with [Virtual key management]({{< link-hextra path="/documentation/llm/cost-controls/virtual-keys/" >}}).
- Review the rate limit policy fields in [Rate limits]({{< link-hextra path="/documentation/configuration/resiliency/rate-limits/" >}}).
