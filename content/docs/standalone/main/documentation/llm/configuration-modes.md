...
Both modes serve traffic on a [gateway]({{< link-hextra path="/documentation/configuration/gateways/" >}}), which defines the port that agentgateway listens on. The difference is how you describe what the gateway serves: the `llm` section describes models, and the `routes` section describes HTTP routes and backends.
...
For more MCP listener context, see [MCP overview]({{< link-hextra path="/documentation/mcp/" >}}).
