...
| Setting | Description |
| --- | --- |
| audiences | The accepted JWT `aud` values. When you set one or more values, the token must have an `aud` claim that matches one of them. Omit `audiences`, or set an empty list, to turn off audience validation. For clients that send tokens without a stable `aud` claim, omit this setting. |
