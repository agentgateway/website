...
OIDC browser authentication provides built-in OpenID Connect login for browser-based clients. By default, unauthenticated browser navigations are redirected to the identity provider's login page, and unauthenticated fetch requests receive a `401 Unauthorized` response instead. You can also send browser navigations to a local login page before the identity provider flow starts, and add a local logout endpoint that clears the gateway session. After successful authentication, the user's session is maintained with encrypted cookies.
