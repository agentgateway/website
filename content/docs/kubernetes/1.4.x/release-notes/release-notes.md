...
The API is disabled by default. To enable it, set the `agentgatewayModels.enabled=true` Helm value on the control plane. For more information, see [About models]({{< link-hextra path="/documentation/llm/models/about/" >}}), [Serve a model]({{< link-hextra path="/documentation/llm/models/serve/" >}}), and [Virtual models]({{< link-hextra path="/documentation/llm/models/virtual/" >}}).
...
Virtual keys can now be sourced from a `ConfigMap` in addition to a `Secret`, and API keys can be stored as SHA-256 hashes so that raw key material never needs to live in the cluster. For more information, see [Virtual keys]({{< link-hextra path="/documentation/llm/cost-controls/virtual-keys/" >}}).
...
For more information, see the [LLM guardrails]({{< link-hextra path="/documentation/llm/guardrails/" >}}) and [MCP guardrails]({{< link-hextra path="/documentation/mcp/guardrails/" >}}) docs.
...
The controller supports `metadataContext`, `requestAttributes`, and `responseAttributes` for external processing, and `extProc` exposes a `failureMode`. For more information, see [External processing]({{< link-hextra path="/documentation/traffic-management/extproc/" >}}).
...
Agentgateway supports the A2A v1.0 agent card format when it rewrites agent card URLs. For more information, see the [A2A]({{< link-hextra path="/documentation/agent/a2a/" >}}) docs.
