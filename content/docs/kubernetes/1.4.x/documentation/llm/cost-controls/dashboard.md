...
1. Create (or update) a Gateway-level {{< reuse "agw-docs/snippets/gatewayparameters.md" >}} resource that enables the database through `rawConfig` and attaches a model catalog. The catalog `configMap` must already exist in the Gateway's namespace (see [Model costs]({{< link-hextra path="/documentation/llm/cost-controls/costs/" >}})).
