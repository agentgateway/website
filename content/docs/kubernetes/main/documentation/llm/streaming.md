...
> [!IMPORTANT]
> Guardrails do not run on streamed responses until you set `promptGuard.streaming` to `Enabled`, and the `Mask` action never applies to a streamed response. Review [Streaming guardrails]({{< link-hextra path="/documentation/llm/guardrails/overview/#streaming" >}}) before you rely on a response guard to protect streamed traffic.
