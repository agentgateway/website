...
The preferred location for MCP authentication is now the route rather than the backend. This aligns MCP authentication with other frontend policies and makes it easier to apply authorization, rate limiting, and transformations based on validated claims.

_Before_: MCP authentication was configured on the {{< reuse "agw-docs/snippets/trafficpolicy.md" >}} targeting an AgentgatewayBackend.
...
_After_: MCP authentication is configured on the {{< reuse "agw-docs/snippets/trafficpolicy.md" >}} targeting an HTTPRoute.
