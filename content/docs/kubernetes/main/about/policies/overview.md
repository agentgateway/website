...
Combine fields in one policy when they share a target and a lifecycle. For the resources that each field can attach to, and for how two policies that set the same field merge, see [Targeting and merging]({{< link-hextra path="/about/policies/target-merge/" >}}).
...
| `backend.mcp` | Configure Model Context Protocol (MCP) authorization, authentication, and guardrails. | [MCP features]({{< link-hextra path="/mcp/" >}}) |
