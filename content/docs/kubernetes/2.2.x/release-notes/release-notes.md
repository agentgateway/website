...
Agentgateway ExtAuth policies now fail closed when the `backendRef` to the auth server is invalid. Previously, invalid backend references might have allowed requests through. Update your [ExtAuth policies]({{< link-hextra path="/documentation/security/extauth/byo-ext-auth-service/" >}}) to ensure backend references are valid before upgrading.

### AI prompt guard API alignment {#ai-prompt-guard}

The [AI prompt guard API]({{< link-hextra path="/documentation/llm/prompt-guards/">}}) is updated to align with other enums. The values changed from `MASK` to `Mask` and `REJECT` to `Reject`. These changes are enforced by CEL validation in the API. Update any existing prompt guard configurations accordingly.
...
**MCP authentication**: [MCP authentication]({{< link-hextra path="/documentation/mcp/auth/" >}}) enables OAuth 2.0 protection for MCP servers, helping to implement the MCP Authorization specification. Agentgateway can act as a resource server, validating JWT tokens and exposing protected resource metadata.
...
* **Basic auth, API key auth, and JWT auth**: Agentgateway proxies now support basic auth, API key auth, and [JWT auth]({{< link-hextra path="/documentation/security/jwt/" >}}).
...
* **CSRF**: Configure [CSRF policies]({{< link-hextra path="/documentation/security/csrf/" >}}) using the `traffic` field in `AgentgatewayPolicy`.
* **ExtAuth**: ExtAuth with HTTP support and configurable timeout.

### Observability

Dynamically configure tracing for agentgateway using the `AgentgatewayPolicy` `frontend` field. See the [tracing setup guide]({{< link-hextra path="/documentation/observability/tracing/">}}).

### mTLS

**TLS encryption for the control plane**: Enable [server-side TLS encryption for the control plane]({{< link-hextra path="/documentation/install/tls/">}}).
