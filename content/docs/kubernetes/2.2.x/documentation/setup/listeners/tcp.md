...
2. Experimental Kubernetes Gateway API features are enabled by default. To set the feature gate explicitly, install or upgrade with the `--set controller.extraEnv.KGW_ENABLE_EXPERIMENTAL_GATEWAY_API_FEATURES=true` Helm flag. For an example, see the [Get started guide]({{< link-hextra path="/documentation/quickstart" >}}).
...
2. Check the status of the Gateway to make sure that your configuration is accepted. Note that in the output, a `NoConflicts` status of `False` indicates that the Gateway is accepted and does not conflict with other Gateway configuration. 
   ```sh
   kubectl get gateway tcp-gateway -n {{< reuse "agw-docs/snippets/namespace.md" >}} -o yaml
   ```
...
1. Get the external address of the gateway and save it in an environment variable.
   ...
   ```sh
   kubectl port-forward deployment/tcp-gateway -n {{< reuse "agw-docs/snippets/namespace.md" >}} 8000:8000
   ```
   {{% /tab %}}
   {{< /tabs >}}
