...
# Source: content/docs/kubernetes/latest/traffic-management/redirect/host.md:29 paths=host-redirect
kubectl apply -f- <<EOF
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
...
# Hidden source: content/docs/kubernetes/latest/traffic-management/redirect/host.md:82 paths=host-redirect
YAMLTest -f - <<'EOF'
...
# Hidden source: content/docs/kubernetes/latest/traffic-management/redirect/host.md:101 paths=host-redirect
for i in $(seq 1 60); do
  curl -s --max-time 5 -o /dev/null "http://${INGRESS_GW_ADDRESS}:80/headers" -H "host: host.redirect.example" && break
  sleep 2
done

# Hidden source: content/docs/kubernetes/latest/traffic-management/redirect/host.md:108 paths=host-redirect
YAMLTest -f - <<'EOF'
