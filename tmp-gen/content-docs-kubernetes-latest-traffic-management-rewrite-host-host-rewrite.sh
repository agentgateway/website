...
# Source: content/docs/kubernetes/latest/traffic-management/rewrite/host.md:29 paths=host-rewrite
kubectl apply -f- <<EOF
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
...
# Hidden source: content/docs/kubernetes/latest/traffic-management/rewrite/host.md:101 paths=host-rewrite
YAMLTest -f - <<'EOF'
...
# Hidden source: content/docs/kubernetes/latest/traffic-management/rewrite/host.md:120 paths=host-rewrite
for i in $(seq 1 60); do
  curl -s --max-time 5 -o /dev/null "http://${INGRESS_GW_ADDRESS}:80/headers" -H "host: rewrite.example" && break
  sleep 2
done

# Hidden source: content/docs/kubernetes/latest/traffic-management/rewrite/host.md:127 paths=host-rewrite
YAMLTest -f - <<'EOF'
