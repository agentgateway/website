...
# Source: content/docs/kubernetes/latest/setup/listeners/http.md:31 paths=http-listener
kubectl apply -f- <<EOF
apiVersion: gateway.networking.k8s.io/v1
kind: Gateway
metadata:
...
# Source: content/docs/kubernetes/latest/setup/listeners/http.md:136 paths=http-listener
kubectl apply -f- <<EOF
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
...
# Hidden source: content/docs/kubernetes/latest/setup/listeners/http.md:179 paths=http-listener
YAMLTest -f - <<'EOF'
...
# Hidden source: content/docs/kubernetes/latest/setup/listeners/http.md:214 paths=http-listener
YAMLTest -f - <<'EOF'
...
# Hidden source: content/docs/kubernetes/latest/setup/listeners/http.md:233 paths=http-listener
for i in $(seq 1 60); do
  curl -s --max-time 5 -o /dev/null "http://${INGRESS_GW_ADDRESS_HTTP}:80/status/200" -H "host: listener.example" && break
  sleep 2
done

# Hidden source: content/docs/kubernetes/latest/setup/listeners/http.md:240 paths=http-listener
YAMLTest -f - <<'EOF'
