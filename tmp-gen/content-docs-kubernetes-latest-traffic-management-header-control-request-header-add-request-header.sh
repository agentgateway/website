...
# Source: content/docs/kubernetes/latest/traffic-management/header-control/request-header.md:51 paths=add-request-header
kubectl apply -f- <<EOF
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
...
# Hidden source: content/docs/kubernetes/latest/traffic-management/header-control/request-header.md:386 paths=add-request-header
YAMLTest -f - <<'EOF'
