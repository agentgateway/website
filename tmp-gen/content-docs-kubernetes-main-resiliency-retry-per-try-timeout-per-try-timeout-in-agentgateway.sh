...
# Source: content/docs/kubernetes/main/resiliency/retry/per-try-timeout.md:175 paths=per-try-timeout-in-agentgateway
kubectl apply -f- <<EOF
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
...
# Source: content/docs/kubernetes/main/resiliency/retry/per-try-timeout.md:200 paths=per-try-timeout-in-agentgateway
kubectl apply -f- <<EOF
...
# Hidden source: content/docs/kubernetes/main/resiliency/retry/per-try-timeout.md:467 paths=per-try-timeout-in-agentgateway
YAMLTest -f - <<'EOF'
