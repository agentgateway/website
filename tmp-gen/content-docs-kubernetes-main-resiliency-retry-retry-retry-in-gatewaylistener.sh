...
# Source: content/docs/kubernetes/main/resiliency/retry/retry.md:284 paths=retry-in-gatewaylistener
kubectl apply -f- <<EOF
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
...
# Source: content/docs/kubernetes/main/resiliency/retry/retry.md:309 paths=retry-in-gatewaylistener
kubectl apply -f- <<EOF
...
# Hidden source: content/docs/kubernetes/main/resiliency/retry/retry.md:476 paths=retry-in-httproute,retry-in-agentgateway,retry-in-gatewaylistener
YAMLTest -f - <<'EOF'
...
- name: Verify that the request was retried. Look for retry.attempt=3 in the output
  retries: 10
  command:
    command: "kubectl logs -n agentgateway-system -l gateway.networking.k8s.io/gateway-name=agentgateway-proxy --tail=1"
  source:
    type: local
  expect:
    exitCode: 0
