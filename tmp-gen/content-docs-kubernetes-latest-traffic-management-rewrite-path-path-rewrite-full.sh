...
# Source: content/docs/kubernetes/latest/traffic-management/rewrite/path.md:294 paths=path-rewrite-full
kubectl apply -f- <<EOF
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
...
# Hidden source: content/docs/kubernetes/latest/traffic-management/rewrite/path.md:384 paths=path-rewrite-full
YAMLTest -f - <<'EOF'
...
# Hidden source: content/docs/kubernetes/latest/traffic-management/rewrite/path.md:403 paths=path-rewrite-full
for i in $(seq 1 60); do
  curl -s --max-time 5 -o /dev/null "http://${INGRESS_GW_ADDRESS}:80/headers" -H "host: rewrite.example" && break
  sleep 2
done

# Hidden source: content/docs/kubernetes/latest/traffic-management/rewrite/path.md:410 paths=path-rewrite-full
YAMLTest -f - <<'EOF'
