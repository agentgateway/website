...
# Source: content/docs/kubernetes/main/resiliency/retry/per-try-timeout.md:69 paths=per-try-timeout-in-httproute
kubectl apply -f- <<EOF
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
...
# Hidden source: content/docs/kubernetes/main/resiliency/retry/per-try-timeout.md:446 paths=per-try-timeout-in-httproute
YAMLTest -f - <<'EOF'
