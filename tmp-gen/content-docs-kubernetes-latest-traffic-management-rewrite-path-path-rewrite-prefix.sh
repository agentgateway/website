...
# Source: content/docs/kubernetes/latest/traffic-management/rewrite/path.md:45 paths=path-rewrite-prefix
kubectl apply -f- <<EOF
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
...
# Hidden source: content/docs/kubernetes/latest/traffic-management/rewrite/path.md:135 paths=path-rewrite-prefix
YAMLTest -f - <<'EOF'
...
# Hidden source: content/docs/kubernetes/latest/traffic-management/rewrite/path.md:154 paths=path-rewrite-prefix
for i in $(seq 1 60); do
  curl -s --max-time 5 -o /dev/null "http://${INGRESS_GW_ADDRESS}:80/headers" -H "host: rewrite.example" && break
  sleep 2
done

# Hidden source: content/docs/kubernetes/latest/traffic-management/rewrite/path.md:161 paths=path-rewrite-prefix
YAMLTest -f - <<'EOF'
